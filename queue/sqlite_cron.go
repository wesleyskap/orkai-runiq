...
func (s *SqliteStorage) RetryModified(ctx context.Context, jobID string, args []byte) error {
	query := `
		...
		WHERE job_id = ?`
	_, err := s.db.ExecContext(ctx, query, args, jobID)
	return err
}

// GetCronSchedules retrieves dynamic cron schedules.
func (s *SqliteStorage) GetCronSchedules(ctx context.Context) ([]CronJob, error) {
	rows, err := s.db.QueryContext(ctx, `SELECT name, spec, queue, payload, timezone, paused FROM runiq_cron_schedules ORDER BY name ASC`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *SqliteStorage) SaveCronSchedule(ctx context.Context, cron CronJob) error {
	...
	_, err := s.db.ExecContext(ctx, query, cron.Name, cron.Spec, cron.Queue, cron.Payload, cron.Timezone, p)
	return err
}
...
func (s *SqliteStorage) DeleteCronSchedule(ctx context.Context, name string) error {
	_, err := s.db.ExecContext(ctx, "DELETE FROM runiq_cron_schedules WHERE name = ?", name)
	return err
}
