...
func (p *PostgresStorage) RetryModified(ctx context.Context, jobID string, args []byte) error {
	query := `
		...
		WHERE job_id = $2`
	_, err := p.db.ExecContext(ctx, query, args, jobID)
	return err
}
...
func (p *PostgresStorage) GetCronSchedules(ctx context.Context) ([]CronJob, error) {
	rows, err := p.db.QueryContext(ctx, `SELECT name, spec, queue, payload, timezone, paused FROM runiq_cron_schedules ORDER BY name ASC`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (p *PostgresStorage) SaveCronSchedule(ctx context.Context, cron CronJob) error {
	...
	_, err := p.db.ExecContext(ctx, query, cron.Name, cron.Spec, cron.Queue, cron.Payload, cron.Timezone, cron.Paused)
	return err
}
...
func (p *PostgresStorage) DeleteCronSchedule(ctx context.Context, name string) error {
	_, err := p.db.ExecContext(ctx, "DELETE FROM runiq_cron_schedules WHERE name = $1", name)
	return err
}
